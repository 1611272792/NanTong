        ...
        public ActionResult GetProductLineData(int ProductLineID, string AndroidID)
        {
            ...
            try
            {
              string  sqltitle = string.Format($"EXEC AboutSelect @type='getAllocationName',@TerminalId={ProductLineID}");
                ...
                string sqlld = string.Format($"select {fieldName} from DataReport where  Issue=1 and Clear=0");
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    for (int j = 1; j <= 45; j++)
                    {
                        try
                        {
                            if (dt.Rows[i]["c" + j].ToString() == "0")
                            {
                                dt.Rows[i]["c" + j] = "--&#000";
                            }
                            else if (dt.Rows[i]["c" + j].ToString() == "1")
                            {
                                dt.Rows[i]["c" + j] = "未开始&#0000ff";
                            }
                            else if (dt.Rows[i]["c" + j].ToString() == "2")
                            {
                                dt.Rows[i]["c" + j] = "进行中&#ff0000";
                            }
                            else if (dt.Rows[i]["c" + j].ToString() == "3")
                            {
                                dt.Rows[i]["c" + j] = "已完成&#6aa84f";
                            }
                        }
                        catch (Exception)
                        {
